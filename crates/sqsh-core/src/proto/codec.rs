...
pub async fn write_message<W, T>(writer: &mut W, msg: &T) -> Result<(), SqshError>
where
    W: AsyncWrite + Unpin,
    T: Serialize,
{
    let payload = to_allocvec(msg).map_err(|e| SqshError::Codec(format!("serialize: {e}")))?;

    let len =
        u32::try_from(payload.len()).map_err(|_| SqshError::Codec("message too large".into()))?;

    writer
        .write_all(&len.to_le_bytes())
        .await
        .map_err(SqshError::Io)?;
    writer.write_all(&payload).await.map_err(SqshError::Io)?;

    Ok(())
}

/// Read a length-prefixed postcard-encoded message from an async reader.
pub async fn read_message<R, T>(reader: &mut R) -> Result<T, SqshError>
where
    R: AsyncRead + Unpin,
    T: for<'de> Deserialize<'de>,
{
    ...
    if len > MAX_FRAME_SIZE {
        return Err(SqshError::Codec(format!(
            "frame too large: {len} bytes (max {MAX_FRAME_SIZE})"
        )));
    }

    let mut buf = vec![0u8; len as usize];
    reader.read_exact(&mut buf).await.map_err(SqshError::Io)?;

    from_bytes(&buf).map_err(|e| SqshError::Codec(format!("deserialize: {e}")))
}
