...
struct Cli {
    /// Key file path (default: ~/.config/sqsh/id_ml_dsa_65)
...
fn generate(key_path: &Path) -> Result<()> {
    if key_path.exists() {
        bail!(
    ...
    let ak_line = sqsh::bootstrap::keys::format_authorized_key(&pubkey, &comment);
...
fn print_pubkey(key_path: &Path) -> Result<()> {
...
mod tests {
    ...
    fn generate_writes_owner_only_file_and_directory() {
        ...
        let key_path = dir.path().join("sqsh").join("id_ml_dsa_65");
