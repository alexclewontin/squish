...
pub async fn run(conn: &Connection, config: &ClientConfig) -> Result<()> {
    ...
    let interactive = config.command.is_none() && config.subsystem.is_none();
    ...
    // Forward locale env (LANG/LC_*) before the shell/exec/subsystem request.
    ...
    let request_type = if let Some(name) = &config.subsystem {
        RequestType::Subsystem { name: name.clone() }
    } else {
...
/// Locale environment variables forwarded to the remote shell.  Only `LANG`
/// and `LC_*` are sent — never arbitrary client env (LD_PRELOAD/BASH_ENV
/// injection risk); the server enforces its own allow-list on top.
pub fn forwarded_env() -> Vec<(String, String)> {
    std::env::vars()
        .filter(|(k, _)| is_forwarded_env_key(k))
        .collect()
}

fn is_forwarded_env_key(k: &str) -> bool {
...
mod tests {
    ...
    #[test]
    fn forwards_only_locale_env() {
        assert!(is_forwarded_env_key("LANG"));
        assert!(is_forwarded_env_key("LC_ALL"));
        assert!(!is_forwarded_env_key("PATH"));
    }
}
