...
use sqsh_core::proto::codec::{read_message, write_message};
use sqsh_core::proto::message::ControlMessage;
...
async fn run_slave_session(mut stream: UnixStream, config: &ClientConfig) -> Result<()> {
    ...
    write_message(
        ...
        &ClientMessage::Open(MasterSessionRequest {
            ...
            pty,
            // ponytail: collect env on the slave, where the user's real environment lives;
            // the long-lived master daemon's env is stale.
            env: crate::channel::session::forwarded_env(),
        }),
    )
    .await?;
