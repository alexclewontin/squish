...
use self::service::SQSHD_INSTALL_PATH;
use self::ssh::SshRunner;

const GITHUB_REPO: &str = "alexclewontin/squish";
const SQSHD_CONFIG_PATH: &str = "/etc/sqsh/sqshd.toml";
...
pub struct BootstrapConfig {
    ...
    pub host: String,
    /// SSH port to use for the bootstrap connection (not the sqshd port).
    pub ssh_port: u16,
    /// Remote sqshd port that will be configured.
    pub sqshd_port: u16,
...
///   2. Download the matching release locally, verify its signed checksums, and upload `sqshd`.
...
pub fn run(cfg: &BootstrapConfig) -> Result<()> {
    ...
    // 2. Ensure sqshd is installed.
    let installed = detect::is_sqshd_installed(&runner).context("checking sqshd install status")?;
    ...
    if !installed {
        ...
        let verified = download_and_verify_sqshd_release(cfg.squishd_version.as_deref(), target)
            .context("downloading and verifying sqshd release artifacts")?;
        eprintln!("[bootstrap] uploading verified sqshd…");
        runner
            .upload(&verified.sqshd_binary, "/tmp/sqshd")
            .context("uploading verified sqshd binary")?;
        runner
            .sudo(&format!("install -m 755 /tmp/sqshd {SQSHD_INSTALL_PATH}"))
            .context("installing sqshd binary")?;
    } else {
        eprintln!("[bootstrap] sqshd already installed, skipping download");
    }

    // 3. Write server config.
    eprintln!("[bootstrap] writing server config…");
    let config_content = build_server_config(cfg.sqshd_port);
    runner
        .sudo("mkdir -p /etc/sqsh && chmod 700 /etc/sqsh")
        .context("creating /etc/sqsh")?;
    runner
        .write_file("/tmp/sqshd.toml", &config_content)
        .context("writing sqshd.toml to /tmp")?;
    runner
        .sudo(&format!(
            "install -m 644 /tmp/sqshd.toml {SQSHD_CONFIG_PATH}"
        ))
        .context("installing sqshd.toml")?;
    ...
    let already_running = detect::is_sqshd_running(&runner).unwrap_or(false);
    if !already_running {
        eprintln!("[bootstrap] installing and starting sqshd service…");
        service::install_and_start(&runner, os).context("installing sqshd service")?;
    } else {
        eprintln!("[bootstrap] sqshd already running, reloading config…");
        reload_service(&runner, os).context("reloading sqshd")?;
    }
    ...
    let host_port = format!("{}:{}", cfg.host, cfg.sqshd_port);
...
fn download_and_verify_sqshd_release(
    version: Option<&str>,
    target: &str,
    ...
    let sqshd_binary = extract_sqshd_binary(&archive_path, staging.path())?;
...
fn extract_sqshd_binary(archive_path: &Path, output_dir: &Path) -> Result<PathBuf> {
    ...
    if !status.success() {
        bail!("failed to extract sqshd from {}", archive_path.display());
    }

    let sqshd_path = output_dir.join("sqshd");
    if !sqshd_path.is_file() {
        bail!("archive {} did not contain sqshd", archive_path.display());
    }
    Ok(sqshd_path)
}
...
fn reload_service(runner: &SshRunner, os: OsKind) -> Result<()> {
    match os {
        OsKind::Linux => runner.sudo("systemctl restart sqshd"),
        OsKind::MacOs => runner.sudo(
            "launchctl unload /Library/LaunchDaemons/com.sqsh.sqshd.plist 2>/dev/null; \
                 launchctl load -w /Library/LaunchDaemons/com.sqsh.sqshd.plist",
        ),
    }
}
