...
/// Check whether sqshd is already installed at the expected path.
pub fn is_sqshd_installed(runner: &SshRunner) -> Result<bool> {
    let result =
        runner.run("command -v sqshd || test -f /usr/local/bin/sqshd && echo yes || echo no");
    match result {
        Ok(s) if s.contains("yes") || s.ends_with("sqshd") => Ok(true),
        Ok(_) => Ok(false),
        Err(_) => Ok(false),
    }
}

/// Check whether the sqshd process is currently running.
pub fn is_sqshd_running(runner: &SshRunner) -> Result<bool> {
    let result = runner.run("pgrep -x sqshd > /dev/null 2>&1 && echo running || echo stopped");
    match result {
