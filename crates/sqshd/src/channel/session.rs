    ...
    loop {
        ...
        match msg {
            ...
            ChannelMessage::Request {
                ...
                return run_child(stream, &user, None, pty_params.take(), &env).await;
            }
            ChannelMessage::Request {
                ...
                return run_child(stream, &user, Some(&command), pty_params.take(), &env).await;
            }
            ChannelMessage::Request {
            ...
            ChannelMessage::Request {
                request_type: RequestType::Env { name, value },
                want_reply,
            } => {
                if config.env_accepted(&name) {
                    // ponytail: cap at 64 entries to bound memory; raise the
                    // limit if a real client needs more locale vars.
                    if env.len() < 64 {
                        env.push((name, value));
                    }
                } else {
                    tracing::debug!(%username, %name, "rejected env var (not in accept_env)");
                }
                if want_reply {
                    stream.sender.send(&ChannelMessage::RequestSuccess).await?;
                }
            ...
            ChannelMessage::Request {
                request_type: RequestType::Subsystem { name },
                want_reply,
            } => match config.resolve_subsystem(&name) {
                Some(command) => {
                    tracing::info!(%username, subsystem = %name, "subsystem request");
                    if want_reply {
                        stream.sender.send(&ChannelMessage::RequestSuccess).await?;
                    }
                    return run_subsystem(stream, &user, &command, &env).await;
                }
                None => {
                    tracing::info!(%username, subsystem = %name, "unknown subsystem");
                    if want_reply {
...
async fn run_child(
    ...
    // Apply privilege drop, env (client then identity), and pre_exec.
    configure_child(&mut cmd, user, env, Some(&term), Some(slave_raw_fd))?;
...
// ---------------------------------------------------------------------------
// Shared child setup: privilege drop, env, and the post-fork pre_exec.
// ---------------------------------------------------------------------------
...
fn configure_child(
    ...
    env: &[(String, String)],
    term: Option<&str>,
    controlling_tty: Option<std::os::fd::RawFd>,
) -> Result<()> {
    // Decide whether we need to switch users. If sqshd's euid already matches
    ...
    if needs_drop && !effective_uid.is_root() {
        bail!(
            "cannot run as '{}': sqshd must be root to switch users (current euid: {})",
            user.name,
            effective_uid
        );
    }

    // Client-supplied env first, so the fixed identity vars below always win
    // (USER/HOME/PATH/etc. can never be overridden by the client).
    for (k, v) in env {
        cmd.env(k, v);
    }
    ...
    unsafe {
        cmd.pre_exec(move || {
            ...
            // Set the controlling terminal only for the PTY path. TIOCSCTTY
            // with arg 0: don't steal if already owned.
            if let Some(fd) = controlling_tty
                && libc::ioctl(fd, libc::TIOCSCTTY as libc::c_ulong, 0i32) < 0
            {
                return Err(std::io::Error::last_os_error());
            }
            Ok(())
...
// ---------------------------------------------------------------------------
// Subsystem (e.g. sftp): no PTY, real pipes, stderr as ExtendedData{1}.
// ---------------------------------------------------------------------------
...
async fn run_subsystem(
    ...
    // ponytail: run via the login shell ("-c") so PATH lookup + arg splitting
    // match OpenSSH's Subsystem semantics; no custom argv parser.
    let mut cmd = std::process::Command::new(&user.shell);
    cmd.arg("-c").arg(command);
    // No TERM, no controlling tty: a PTY would corrupt the binary protocol.
    configure_child(&mut cmd, user, env, None, None)?;
    ...
    let mut child = Command::from(cmd)
        .kill_on_drop(true)
        .spawn()
        .context("failed to spawn subsystem")?;

    let mut child_stdin = child.stdin.take();
    let mut child_stdout = child.stdout.take().context("subsystem stdout missing")?;
    let mut child_stderr = child.stderr.take().context("subsystem stderr missing")?;
    ...
    // Terminate on child stdout EOF so the binary stream is fully drained
    // before we tear down (child.wait() racing a final read would lose bytes).
    loop {
        tokio::select! {
            // child stdout → Data
            r = child_stdout.read(&mut out_buf) => {
                match r {
                    Ok(0) | Err(_) => break,
                    Ok(n) => {
                        let data = out_buf[..n].to_vec();
            ...
            // child stderr → ExtendedData{1}
            r = child_stderr.read(&mut err_buf), if !stderr_done => {
                match r {
                    Ok(0) | Err(_) => stderr_done = true,
                    Ok(n) => {
                        let data = err_buf[..n].to_vec();
                        stream
                            .sender
                            .send(&ChannelMessage::ExtendedData { data_type: 1, data })
                            .await
                            .ok();
                    }
                }
            }
            // client → child stdin / control
            msg = stream.receiver.recv() => {
                match msg {
                    Ok(ChannelMessage::Data { data }) => {
                        let write_failed = match child_stdin.as_mut() {
                    ...
                    Ok(ChannelMessage::Eof) => {
                        // Client finished sending — close child stdin so it sees EOF.
                        child_stdin = None;
                    }
            ...
            // child exit: capture status, keep draining stdout until EOF.
    ...
    let exit_status = match status {
        Some(s) => exit_code_from_status(s),
        // ponytail: killed or stream dropped before reap → generic failure code.
        None => child.wait().await.map(exit_code_from_status).unwrap_or(255),
    };

    stream.sender.send(&ChannelMessage::Eof).await.ok();
    stream
        .sender
        .send(&ChannelMessage::ExitStatus {
            status: exit_status,
    ...
    tracing::info!(username = %user.name, exit_status, "subsystem ended");
    Ok(())
}
