...
#[derive(Parser)]
#[command(name = "qsshd", about = "QSSH server daemon")]
struct Cli {
    /// Path to config file
    #[arg(short, long, default_value = "/etc/qssh/qsshd.toml")]
    config: String,
...
async fn main() -> Result<()> {
    ...
    if cli.emit_fingerprint {
        ...
        let fp = qssh_core::auth::fingerprint::cert_fingerprint(cert_der.as_ref());
